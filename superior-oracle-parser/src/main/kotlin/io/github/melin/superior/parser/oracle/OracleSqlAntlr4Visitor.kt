...
class OracleSqlAntlr4Visitor(
    ...
    override fun visitCreate_database(
        ctx: OracleParser.Create_databaseContext
    ): Statement {
        val databaseName = CommonUtils.cleanQuote(ctx.database_name().text)
        return CreateDatabase(databaseName)
    }

    override fun visitCreate_table(
        ctx: OracleParser.Create_tableContext
    ): Statement {
        currentOptType = StatementType.CREATE_TABLE
        val schemaName =
            if (ctx.schema_name() != null) ctx.schema_name().text else null
            ...
            ctx.relational_table()
                ...
                .map {
                    ...
                    val defaultExpr: String? =
                        if (colDef.expression() != null)
                            colDef.expression().text
                        else null
    ...
    override fun visitCreate_view(
        ctx: OracleParser.Create_viewContext
    ): Statement {
        ...
        val queryStmt =
            this.parseSelect_only_statement(ctx.select_only_statement())
                as QueryStmt
    ...
    override fun visitCreate_materialized_view(
        ctx: OracleParser.Create_materialized_viewContext
    ): Statement {
    ...
    override fun visitDrop_function(
        ctx: OracleParser.Drop_functionContext
    ): Statement {
        val funcName = ctx.function_name()
        val functionId = parseFunctionName(funcName)
    ...
    override fun visitDrop_procedure(
        ctx: OracleParser.Drop_procedureContext
    ): Statement {
        val procedureName = ctx.procedure_name()
        val procedureId = parseProcedureName(procedureName)
    ...
    override fun visitAlter_table(
        ctx: OracleParser.Alter_tableContext
    ): Statement {
        val tableId = parseTableViewName(ctx.tableview_name())
        return AlterTable(tableId)
    }

    override fun visitAlter_view(
        ctx: OracleParser.Alter_viewContext
    ): Statement {
        val tableId = parseTableViewName(ctx.tableview_name())
    ...
    override fun visitSelect_statement(
        ctx: OracleParser.Select_statementContext
    ): Statement {
    ...
    private fun parseSelect_only_statement(
        ctx: OracleParser.Select_only_statementContext
    ): Statement {
        currentOptType = StatementType.SELECT
    ...
    override fun visitDelete_statement(
        ctx: OracleParser.Delete_statementContext
    ): Statement {
        currentOptType = StatementType.DELETE
        val tableId =
            parseTableViewName(
                ctx.general_table_ref()
    ...
    override fun visitUpdate_statement(
        ctx: OracleParser.Update_statementContext
    ): Statement {
    ...
    override fun visitInsert_statement(
        ctx: OracleParser.Insert_statementContext
    ): Statement {
        ...
        val insertTable =
            if (ctx.single_table_insert() != null) {
                ...
                if (tableInsert.select_statement() != null) {
                    val queryStmt =
                        this.visitSelect_statement(
                            tableInsert.select_statement()
                        ) as QueryStmt
                ...
                if (
                    ctx.multi_table_insert().conditional_insert_clause() == null
                ) {
                    val tableInserts =
                        ctx.multi_table_insert().multi_table_element()
                    addOutputTableId(tableInserts)
                } else {
                    ctx.multi_table_insert()
                        .conditional_insert_clause()
                        .conditional_insert_when_part()
                        .map { addOutputTableId(it.multi_table_element()) }
                ...
                if (ctx.multi_table_insert().select_statement() != null) {
                    val queryStmt =
                        this.visitSelect_statement(
                            ctx.multi_table_insert().select_statement()
                        ) as QueryStmt
                    InsertTable(
                        InsertMode.INTO,
                        queryStmt,
    ...
    private fun parseProcedureName(
        procedureName: Procedure_nameContext
    ): ProcedureId {
        return if (procedureName.id_expression() != null) {
            ProcedureId(
                procedureName.identifier().text,
                procedureName.id_expression().text
            )
        } else {
    ...
    override fun visitMerge_statement(
        ctx: OracleParser.Merge_statementContext
    ): Statement {
    ...
    override fun visitComment_on_column(
        ctx: OracleParser.Comment_on_columnContext
    ): Statement {
    ...
    override fun visitComment_on_table(
        ctx: OracleParser.Comment_on_tableContext
    ): Statement {
    ...
    override fun visitComment_on_materialized(
        ctx: OracleParser.Comment_on_materializedContext
    ): Statement {
        val objValue = ctx.tableview_name().text
        val isNull = false
        val text: String = CommonUtils.cleanQuote(ctx.quoted_string().text)
    ...
    override fun visitSubquery_factoring_clause(
        ctx: OracleParser.Subquery_factoring_clauseContext
    ): Statement? {
        cteTempTables.add(TableId(ctx.query_name().text))
        return super.visitSubquery_factoring_clause(ctx)
    }

    override fun visitOffset_clause(
        ctx: OracleParser.Offset_clauseContext
    ): Statement? {
        try {
    ...
    override fun visitFetch_clause(
        ctx: OracleParser.Fetch_clauseContext
    ): Statement? {
        try {
    ...
    override fun visitTableview_name(
        ctx: OracleParser.Tableview_nameContext
    ): Statement? {
        if (
            currentOptType == StatementType.SELECT ||
                currentOptType == StatementType.CREATE_VIEW ||
    ...
    private fun parseTableViewName(
        ctx: OracleParser.Tableview_nameContext
    ): TableId {
        if (ctx.childCount == 1) {
