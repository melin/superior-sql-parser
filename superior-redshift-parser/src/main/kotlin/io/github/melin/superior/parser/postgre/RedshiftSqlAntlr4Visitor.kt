 ...
 */
class RedshiftSqlAntlr4Visitor(val splitSql: Boolean = false, val command: String?): RedshiftParserBaseVisitor<Statement>() {
    ...
    override fun visitCreatefunctionstmt(ctx: RedshiftParser.CreatefunctionstmtContext): Statement {
        ...
        if (optItems != null) {
            optItems.filter { it.func_as() != null && it.func_as().Definition != null}.forEach {
                visitSqlroot(it.func_as().Definition as RedshiftParser.SqlrootContext)
            }
        }
