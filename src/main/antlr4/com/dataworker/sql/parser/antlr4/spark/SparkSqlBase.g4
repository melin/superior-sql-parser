...
statement
    ...
    | (DESC | DESCRIBE) DETAIL table=multipartIdentifier               #describeDeltaDetail
    | (DESC | DESCRIBE) HISTORY table=multipartIdentifier
    ...
    | MERGE TABLE multipartIdentifier partitionSpec?
        (OPTIONS options=tablePropertyList)?                           #mergeTable
    | KILL JOB  identifier                                             #killJob
    | ADDJAR jar=identifier                                            #addJar
    | READ TABLE multipartIdentifier partitionSpec? LIMIT number       #readTable
    ...
    | CONVERT TO format = (DELTA | PARQUET) table=multipartIdentifier
        (PARTITIONED BY '(' colTypeList ')')?
        (OPTIONS options=tablePropertyList)?                           #deltaConvert
