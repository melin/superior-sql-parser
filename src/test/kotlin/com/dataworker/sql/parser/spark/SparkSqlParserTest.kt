...
class SparkSqlParserTest {
    ...
    fun dataXTest() {
        val sql = """
            datax reader('sftp') options('host'='x.x.x.x') writer('hive') options(ss.table='demo', columns="id,name,age")
        """
        val statementData = SparkSQLHelper.getStatementData(sql)
        val statement = statementData.statement
        if (statement is DataxExpr) {
            ...
            Assert.assertEquals("demo", statement.distOptions.get("ss.table"))
            Assert.assertEquals("id,name,age", statement.distOptions.get("columns"))
        } else {
            Assert.fail()
        }
    }
