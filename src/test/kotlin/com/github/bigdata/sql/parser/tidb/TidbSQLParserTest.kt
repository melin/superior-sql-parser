...
import com.github.bigdata.sql.parser.spark.SparkSQLHelper
...
class TidbSQLParserTest {
    ...
    @Test
    fun renameTableTest() {
        val sql = "RENAME TABLE `bigdata`.`users_quan` TO  `bigdata`.`dc_users`\n"
        val statementData = TidbSQLHelper.getStatementData(sql)
        val statement = statementData.statement
    ...
    fun selectTest0() {
        ...
        val tableData = TidbSQLHelper.getStatementData(sql)
    ...
    fun changeColumnTest() {
        val sql = "ALTER TABLE `bigdata`.`log_collect_config` \n" +
    ...
    @Test
    fun addColumnTest() {
        val sql = "ALTER TABLE `bigdata`.`users_quan` ADD COLUMN `age` VARCHAR(45) NULL DEFAULT 18 COMMENT '年龄' AFTER `username`"
        val statementData = TidbSQLHelper.getStatementData(sql)
        val statement = statementData.statement
        if(statement is TableSource) {
    ...
    @Test
    fun dropColumnTest() {
        val sql = "ALTER TABLE `bigdata`.`users_quan` DROP COLUMN `age`;"
        val statementData = TidbSQLHelper.getStatementData(sql)
        val statement = statementData.statement
        if(statement is TableSource) {
    ...
    @Test
    fun addUNIQUETest() {
        val sql = "ALTER TABLE `bigdata`.`dc_project_member`\n" +
    ...
    fun dropIndexTest1() {
        ...
        val sqls = TidbSQLHelper.splitAlterSql(sql);
        val statementData = TidbSQLHelper.getStatementData(sqls.get(0))
    ...
    fun splitSqlTest3() {
        ...
        val statement = TidbSQLHelper.getStatementData(sqls.get(0))
